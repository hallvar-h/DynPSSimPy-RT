...
from topsrt.gui import LineOutageWidget, VSCControlWidget, DynamicLoadControlWidget
...
class LoadActivePowerPlot(RTSimPlot):
    ...
    def get_init_data(rts):
        return rts.ps.loads['DynamicLoadFiltered'].n_units

    @staticmethod
    def read_input_signal(rts):
        return rts.sol.t, rts.ps.loads['DynamicLoadFiltered'].p(rts.sol.x, rts.sol.v)
    
class LoadReactivePowerPlot(LoadActivePowerPlot):
    @staticmethod
    def read_input_signal(rts):
        return rts.sol.t, rts.ps.loads['DynamicLoadFiltered'].q(rts.sol.x, rts.sol.v)
...
def main(rts):
    ...
    load_control = DynamicLoadControlWidget(rts, 'DynamicLoadFiltered')
