...
class InterfacerQueues:
    def __init__(self, rts=None, name='InterfacerQueues', fs=None, *args, **kwargs):
        ...
        if rts:
            self.connect(rts)
            print('Added interface {} to RealTimeSimulator.'.format(self.interface_name))
    ...
    def connect(self, rts):
        ...
        interface_name_unique = self.interface_name.copy()
        counter = 2
...
class InterfacerQueuesThread(rts_utils.MakeThread, InterfacerQueues):
    def __init__(self, *args, **kwargs):
        InterfacerQueues.__init__(self, *args, **kwargs)
        rts_utils.MakeThread.__init__(self, *args, **kwargs)


class InterfacerQueuesProcess(rts_utils.MakeProcess, InterfacerQueues):
    def __init__(self, *args, **kwargs):
        InterfacerQueues.__init__(self, *args, **kwargs)
        rts_utils.MakeProcess.__init__(self, *args, **kwargs)
...
class InterfaceListener(rts_utils.MakeThread):
    ...
    def main_loop(self):
        while not self.stopped():
            ...
            def interface_fun_no_kwargs(rts, read_input_signal=read_input_signal_rebuilt,
